...
/**
 * Tomda协议解码类 Created by jiafeng on 2018/09/09.
 */
public class TomdaChargerDecoder extends ByteToMessageDecoder {
	...
	public void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
		...
		logger.info((Tool.formatDateTime(new Date()) + "Tomda协议,终端" + chargerInfo + "上报的命令length = " + length
				+ ", show content = " + Tool.showData(content)));
		System.out.println((Tool.formatDateTime(new Date()) + "Tomda协议,终端" + chargerInfo + "上报的命令length = " + length
				+ ", show content = " + Tool.showData(content)));
		try {
			// 判断协议开头
			if (!content.startsWith(TomdaProtocol.PREFIX_TEXT)) {
				...
				if (protocolHeaderIndex > 0) {
					...
					logger.info("Tomda协议,服务接收到终端" + chargerInfo + "的指令协议头不对,但指令中有包含协议头，丢弃内容长度[" + discardLength
							+ "],丢弃内容：" + Tool.showData(decode(discardBuf.nioBuffer())));
					System.out.println("Tomda协议,服务接收到终端" + chargerInfo + "的指令协议头不对,但指令中有包含协议头，丢弃内容长度[" + discardLength
							+ "],丢弃内容：" + Tool.showData(decode(discardBuf.nioBuffer())));
			...
			int validLength = PTool.count(content.charAt(TomdaProtocol.BIT_DATA_LENGTH + 1),
					content.charAt(TomdaProtocol.BIT_DATA_LENGTH));
			...
			if (length >= validLength) {
				...
				if (checksumCalculate == checksumRead) { // 检测校验和是否合格
					...
					logger.error("Tomda协议,终端" + chargerInfo + "上报的非法帧：校验和校验失败，checksum为 " + checksumRead
							+ ", checksumCalculate为 " + checksumCalculate);
					...
					if (protocolHeaderIndex > 0) {
						...
						ByteBuf discardBuf = buffer.readBytes(discardLength);
						logger.info("Tomda协议,服务接收到终端" + chargerInfo + "的指令不符合规范，丢弃内容长度[" + validLength + "],丢弃内容："
								+ Tool.showData(decode(discardBuf.nioBuffer())));
						discardBuf.release(); // 用完即释放
					} else {
...
//            ReferenceCountUtil.release(buffer);
	...
	public MessageLite decodeBody(int dataType, byte[] data1) throws Exception {
		...
		case 303:
			return ChargingDataReportProto.chargingDataReportReq.getDefaultInstance().getParserForType()
					.parseFrom(data); // 工作数据上报
		case 304:
			return ChargingDataReportProto.chargingDataReportResp.getDefaultInstance().getParserForType()
					.parseFrom(data); // 工作数据上报应答
		case 305:
		...
		case 307:
			return StatusNotificationProto.statusNotificationReq.getDefaultInstance().getParserForType()
					.parseFrom(data); // 插头状态上报
		case 308:
			return StatusNotificationProto.statusNotificationResp.getDefaultInstance().getParserForType()
					.parseFrom(data); // 插头状态上报应答
		case 401:
		...
		case 501:
			return UpdateFirmwareEventProto.updateFirmwareEventResp.getDefaultInstance().getParserForType()
					.parseFrom(data); // 固件更新请求应答
		case 502:
			return UpdateFirmwareEventProto.updateFirmwareEventReq.getDefaultInstance().getParserForType()
					.parseFrom(data); // 固件更新请求
		case 503:
			return FirmwareStatusNotificationProto.firmwareStatusNotificationReq.getDefaultInstance().getParserForType()
					.parseFrom(data); // 固件更新状态上报
		case 504:
			return FirmwareStatusNotificationProto.firmwareStatusNotificationResp.getDefaultInstance()
					.getParserForType().parseFrom(data); // 固件更新状态上报应答
		case 505:
